...
class VNTM(nn.Module):
    ...
    def forward(self, x, input, device, n_sample=1, epoch = 0):
        h = self.hidden(input)
        h = self.drop(h)
...
class TopicModel:
    ...
    def train(self, X, Input, batch_size):
        self.model.train()
        total_nll = 0.0
        total_kld = 0.0
        ...
        for idx, ind in enumerate(indices):
            ...
            emb_batch = Input[ind].to(self.device).float()
            d = self.model(x = data_batch, input = emb_batch,  device = self.device)
    ...
    def fit_transform(self, dataset, name, index = [], **kargs):
        ...
        layer = embedding_text.shape[1]//16
        hidden = get_mlp([embedding_text.shape[1], embedding_text.shape[1]//4, layer], nn.GELU)
        ...
        for epoch in range(self.epochs):
            self.train(bag_of_words, embedding_text, self.batch_size)
        ...
        z = self.model.hidden(embedding_text.float())
    ...
    def train_model(self, dataset, name, hyperparameters={}, top_words=10):
